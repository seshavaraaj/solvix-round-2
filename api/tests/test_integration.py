...
def test_event_surge_flow(client, op):
    ...
    move = next(r for r in cyc["recommendations"] if r["action"] == "move_bus" and r["to_route_id"] == "534")
    ...
    n_before = sum(b["route_id"] == "534" for b in before["buses"])
    ...
    assert sum(b["route_id"] == "534" for b in after["buses"]) == n_before + move["bus_count"]
    alerts = client.get("/alerts", params={"route_id": "534"}).json()
...
def test_rider_crowded_reports_raise_route_load(client, op, rider):
    """M4: rider "Crowded" on Route 534 raises its load on the operator map."""
    ...
    target = next(b for b in before.values() if b["route_id"] == "534" and b["load_factor"] > 0.2)
    r = client.post("/crowding", json={"bus_id": target["id"], "route_id": "534", "level": "crowded"},
                    headers=rider)
    assert r.status_code == 201
    assert client.post("/crowding", json={"bus_id": target["id"], "route_id": "534", "level": "crowded"},
...
def test_admin_fleet_validation(client, admin):
    bad = {"depot_id": "depot_okhla", "name": "Okhla Depot", "lat": 28.53, "lon": 77.27, "fleet_size": 3,
           "reserve": 4, "out_of_service": 1}
    assert client.put("/admin/fleet/depot_okhla", json=bad, headers=admin).status_code == 400
    good = {**bad, "fleet_size": 30}
    assert client.put("/admin/fleet/depot_okhla", json=good, headers=admin).json() == good
